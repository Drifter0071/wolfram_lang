...
fn build_completions(
    ...
    if ctx == Ctx::ImportPath {
        let partial = extract_import_partial(&line_prefix).unwrap_or_default().to_lowercase();
            ...
            if ch == ':' {
                for method in bindings.get_all_methods(&type_name) {
                    ...
                    items.push(CompletionItem {
                        ...
                        documentation: if method.description.is_empty() { None } else { Some(Documentation::MarkupContent(MarkupContent { kind: MarkupKind::Markdown, value: method.description.clone() })) },
                        ..Default::default()
                    });
                }
            } else {
                for prop in bindings.get_all_properties(&type_name) {
                    ...
                    items.push(CompletionItem {
                        ...
                        documentation: if prop.description.is_empty() { None } else { Some(Documentation::MarkupContent(MarkupContent { kind: MarkupKind::Markdown, value: prop.description.clone() })) },
                        ..Default::default()
                    });
                }
                for method in bindings.get_all_methods(&type_name) {
    ...
    if add_locals {
        for (name, var_type) in &scope.variables {
            if name.to_lowercase().starts_with(wp) {
                let detail = if var_type != "any" && var_type != "local" && var_type != name { var_type.clone() } else { String::new() };
                items.push(CompletionItem {
                    label: name.clone(),
    ...
    if add_keywords {
        let struct_kw = ["if", "else", "elif", "while", "for", "function", "class", "struct", "enum", "import", "local", "return", "break", "continue", "public", "private"];
        for &k in &struct_kw {
            if k.starts_with(wp) {
                let mut item = CompletionItem { label: k.into(), kind: Some(CompletionItemKind::KEYWORD), sort_text: Some(format!("0{}", k)), ..Default::default() };
                match k {
                    "if" => { item.insert_text = Some("if (${1:condition}) {\n\t${0}\n}".into()); item.insert_text_format = Some(InsertTextFormat::SNIPPET); }
                    "else" => { item.insert_text = Some("else {\n\t${0}\n}".into()); item.insert_text_format = Some(InsertTextFormat::SNIPPET); }
                    "elif" => { item.insert_text = Some("elif (${1:condition}) {\n\t${0}\n}".into()); item.insert_text_format = Some(InsertTextFormat::SNIPPET); }
    ...
    if add_value_kw {
        ...
        for &k in &val_kw {
            if k.starts_with(wp) {
                items.push(CompletionItem { label: k.into(), kind: Some(CompletionItemKind::KEYWORD), sort_text: Some(format!("0{}", k)), ..Default::default() });
            }
        }
    }
        ...
        for f in &bindings.functions {
            if f.name.to_lowercase().starts_with(wp) {
                ...
                items.push(CompletionItem {
                    ...
                    documentation: if f.description.is_empty() { None } else { Some(Documentation::MarkupContent(MarkupContent { kind: MarkupKind::Markdown, value: f.description.clone() })) },
                    ..Default::default()
                });
            }
        }
            ...
            for en in &bindings.enums {
                if en.name.to_lowercase().starts_with(wp) {
                    items.push(CompletionItem {
                        ...
                        documentation: if en.description.is_empty() { None } else { Some(Documentation::MarkupContent(MarkupContent { kind: MarkupKind::Markdown, value: en.description.clone() })) },
                        ..Default::default()
                    });
                }
            }
...
fn detect_context(line_prefix: &str) -> Ctx {
    ...
    for ch in line_prefix.chars() {
        if !in_string && (ch == '"' || ch == '\'') { in_string = true; quote_char = ch; }
        else if in_string && ch == quote_char { in_string = false; }
    }
    if in_string {
    ...
    if re.is_match(trimmed) { return Ctx::ImportPath; }

    // Definition name: function/class/struct/enum keyword followed by (optional) name
    let def_re = regex_lite::Regex::new(r"^(?:local\s+)?(?:public\s+|private\s+)?(function|class|struct|enum)\s+\w*$").unwrap();
    if def_re.is_match(trimmed) { return Ctx::DefinitionName; }

    // Dot/colon
    let last_char = line_prefix.chars().last().unwrap_or(' ');
    if last_char == '.' || last_char == ':' {
        if trimmed.ends_with("Enum.") { return Ctx::EnumValue; }
        return Ctx::DotColon;
    }
    ...
    if let Some(word) = word_before_cursor(trimmed) {
        if word.to_lowercase().starts_with("enum.") {
            ...
            if is_value_position(before) { return Ctx::EnumValue; }
        }
    }

    // Value expression
    if is_value_position(trimmed) { return Ctx::ValueExpr; }
...
fn word_before_cursor(text: &str) -> Option<&str> {
    ...
    while end > 0 && !(bytes[end - 1] as char).is_alphanumeric() && bytes[end - 1] != b'_' && bytes[end - 1] != b'.' {
        end -= 1;
    }
    if end == 0 { return None; }
    let mut start = end;
    while start > 0 && ((bytes[start - 1] as char).is_alphanumeric() || bytes[start - 1] == b'_' || bytes[start - 1] == b'.') {
        start -= 1;
    }
    Some(&text[start..end])
}
...
pub fn handle_definition(
    ...
    for other_doc in store.get_all() {
        if let Some(sym) = other_doc.symbols.iter().find(|s| s.name == word && s.access == "public") {
            let range = Range {
                start: Position { line: (sym.location.line as u32).saturating_sub(1), character: (sym.location.column as u32).saturating_sub(1) },
                end: Position { line: (sym.location.end_line as u32).saturating_sub(1), character: (sym.location.end_column as u32).saturating_sub(1) },
            };
...
fn extract_extended_expr_at(source: &str, pos: &Position) -> String {
    ...
    let start = line[..col].rfind(|c: char| !c.is_alphanumeric() && c != '_' && c != '.').map(|i| i + 1).unwrap_or(0);
    let end = line[col..].find(|c: char| !c.is_alphanumeric() && c != '_' && c != '.').map(|i| col + i).unwrap_or(line.len());
...
pub fn handle_diagnostics(
    ...
    for warning in &scope_warnings {
        ...
        diagnostics.push(Diagnostic {
            range: Range {
                start: Position { line: (line as u32).saturating_sub(1), character: (col as u32).saturating_sub(1) },
                end: Position { line: (line as u32).saturating_sub(1), character: col as u32 + 30 },
            },
...
pub fn handle_signature_help(
    ...
    let offset = lines[..pos.line as usize].iter().map(|l| l.len() + 1).sum::<usize>() + col;
    ...
    let (label, params, ret, doc_text, has_overload) = find_signature(bindings, &func_name, &doc.scope, full_source);
    ...
    let label_str = if ret.is_empty() {
        format!("{}({})", label, params.iter().map(|(n, t)| if t.is_empty() { n.clone() } else { format!("{}: {}", n, t) }).collect::<Vec<_>>().join(", "))
    } else {
        format!("{}({}) -> {}", label, params.iter().map(|(n, t)| if t.is_empty() { n.clone() } else { format!("{}: {}", n, t) }).collect::<Vec<_>>().join(", "), ret)
    };
    ...
    Some(SignatureHelp {
        signatures: vec![SignatureInformation {
            ...
            active_parameter: if has_overload { Some(active as u32) } else { None },
        }],
...
fn find_signature(
    ...
    if let Some(colon_pos) = func_name.rfind(':') {
        ...
        if let Some(type_name) = resolve_expression_type(obj_part, bindings, scope, source) {
            if let Some(m) = bindings.get_all_methods(&type_name).iter().find(|m| m.name.eq_ignore_ascii_case(method)) {
                let params: Vec<(String, String)> = m.params.iter()
                    .map(|p| (format!("self: {}", p.name), p.r#type.clone()))
                    .collect();
                let has_over = bindings.get_all_methods(&type_name).iter().filter(|m2| m2.name.eq_ignore_ascii_case(method)).count() > 1;
                return (format!("{}.{}", type_name, method), params, m.returns.clone(), m.description.clone(), has_over);
            }
        }
        let params = vec![("self".into(), String::new()), ("...".into(), String::new())];
        return (func_name.to_string(), params, String::new(), String::new(), false);
    }
    ...
    if parts.len() > 1 {
        let class = parts[0];
        let method = parts[1];
        if let Some(m) = bindings.get_all_methods(class).iter().find(|m| m.name.eq_ignore_ascii_case(method)) {
            let params: Vec<(String, String)> = m.params.iter()
                .map(|p| (p.name.clone(), p.r#type.clone()))
                .collect();
            let has_over = bindings.get_all_methods(class).iter().filter(|m2| m2.name.eq_ignore_ascii_case(method)).count() > 1;
            return (func_name.to_string(), params, m.returns.clone(), m.description.clone(), has_over);
        }
    }

    // Simple function call
    if let Some(f) = bindings.functions.iter().find(|f| f.name.eq_ignore_ascii_case(func_name)) {
        let params: Vec<(String, String)> = f.params.iter()
            .map(|p| (p.name.clone(), p.r#type.clone()))
            .collect();
        let has_over = bindings.functions.iter().filter(|f2| f2.name.eq_ignore_ascii_case(func_name)).count() > 1;
        return (f.name.clone(), params, f.returns.clone(), f.description.clone(), has_over);
    }

    // Fallback with no args
    (func_name.to_string(), vec![], String::new(), String::new(), false)
}
