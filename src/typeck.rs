...
impl TypeChecker {
    pub fn check(stmts: &[Stmt]) -> TypeCheckResult {
        ...
        for g in ROBLOX_GLOBALS {
            checker.scopes[0]
                .vars
                .insert(g.to_string(), InferredType::Instance);
        }
        ...
        checker.scopes[0]
            .vars
            .insert("print".into(), InferredType::Function);
        checker.scopes[0]
            .vars
            .insert("warn".into(), InferredType::Function);
        checker.scopes[0]
            .vars
            .insert("error".into(), InferredType::Function);
    ...
    fn walk_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::FuncDef {
                ...
                self.push_scope();
                for p in params {
                    self.declare(p, InferredType::Unknown);
                }
            ...
            Stmt::Import { alias, .. } => {
                self.declare(alias, InferredType::Table);
            }
            Stmt::TryCatch {
                try_block,
                catch_clauses,
                ...
                for (_, var_name, block) in catch_clauses {
                    ...
                    if let Some(v) = var_name {
                        self.declare(v, InferredType::String);
                    }
                    self.walk_stmts(block);
                    self.pop_scope();
                }
        ...
        match expr {
            ...
            Expr::UnaryMinus(e) => {
                ...
                if inner != InferredType::Number && inner != InferredType::Unknown {
                    self.result.warnings.push(format!(
                        "unary minus applied to '{}' (expected number)",
                        inner.name()
                    ));
                }
                InferredType::Number
            ...
            Expr::MethodCall {
                ...
                if field == "new"
                    && obj_ty != InferredType::Unknown
                    && obj_ty != InferredType::Table
                {
                    return obj_ty;
                }
                InferredType::Unknown
    ...
    fn check_binary(
        ...
        match op {
            ...
            ".." => {
                if left != &InferredType::Unknown && right != &InferredType::Unknown {
                    if left != &InferredType::String
                        && right != &InferredType::String
                        && left != &InferredType::Number
                        && right != &InferredType::Number
                    {
                        self.result.warnings.push(format!(
